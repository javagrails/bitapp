...
class LoginController {
    ...
    def loginSuccess(){
        //String url = '/'
        //String url = '/apple/index'
        String url = '/app/dashboard'
        log.info("loginSuccess url => "+url)

        if (springSecurityService.isLoggedIn()) {
            session.issue = true;
            session.reqMaps = securityService.buildMenu(session?.SPRING_SECURITY_CONTEXT?.authentication?.principal?.id)
     ...
     */
    def denied() {
        def message = "You have no access permission in requested feature"
        render(view: "denied", model: [message: message])
    }
